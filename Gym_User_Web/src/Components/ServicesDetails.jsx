  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const serviceData = serviceResponse.data.find((s) => s.slug === slug);
        ...
        if (serviceData) {
          ...
          const trainersResponse = await api.get("/staff");
          const allTrainers = trainersResponse.data;

          // Filter trainers by service slug if they have services field
          const related = allTrainers.filter((t) =>
            t.services?.some(
