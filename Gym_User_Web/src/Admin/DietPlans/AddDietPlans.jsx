...
const AddDietPlans = () => {
  ...
  useEffect(() => {
    const fetchAllMembers = async () => {
      try {
        ...
        const formatted = memberList.map((m, index) => {
          const membershipId = m.id || m.memberId || m.membershipId || m.userId || index;
          const rawMemberId = m.memberId || m.member_id || null;
          const validMemberId = rawMemberId && String(rawMemberId).trim() !== "0" ? String(rawMemberId).trim() : null;
          const rawUserId = m.userId || m.user_id || null;
        ...
        console.log("📋 Admin fetched members:", formatted.length);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        ...
        setTimeout(() => navigate("/admin/alladddietplans"), 1200);
      } else {
        // Bulk Create
  ...
  const baseDate = id && form.createdAt ? dayjs(form.createdAt) : dayjs();
            ...
            <div className="bg-black/40 border border-white/10 rounded-xl p-4 space-y-4">
              <div className="flex items-center justify-between">
                <label className="text-sm font-semibold flex items-center gap-2">
                  ...
                  Select Members ({selected.size} / {filteredMembers.length})
                </label>
                <div
                  onClick={selectAll}
                  ...
                  filteredMembers.map((m) => {
                    ...
                    return (
                      <div
                        key={m.id}
                        onClick={() => toggleOne(m)}
                        className={`flex items-center gap-3 p-3 rounded-lg transition border ${isSelected ? "bg-emerald-500/20 border-emerald-500/50" : "bg-white/5 border-white/5 hover:bg-white/10 cursor-pointer"}`}
                      >
                        {isSelected ? (
