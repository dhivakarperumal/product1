...
(async () => {
  ...
  // Show final result
  const [allPlans] = await conn.query('SELECT id, plan_id, name, duration, price, created_by FROM gym_plans ORDER BY id');
  console.log('\n' + '='.repeat(130));
  console.log('All Plans in UUID Format:');
  console.log('='.repeat(130));
  allPlans.forEach(p => {
    const createdBy = p.created_by || 'NULL';
    console.log(`ID: ${String(p.id).padEnd(3)} | plan_id: ${p.plan_id} | name: ${String(p.name).padEnd(15)} | duration: ${String(p.duration).padEnd(10)} | price: ${String(p.price).padEnd(8)} | created_by: ${createdBy}`);
  });
  console.log('='.repeat(130));
